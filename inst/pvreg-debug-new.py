...
def load_data(in_estimates_df, in_responses_df,
    ...
    ''' Returns:
        ...
        (3) dict from item responses and grouping variable; key=(item.name,item_response,group.name), value = index
    ...
    estimates_df = pd.concat([in_estimates_df, out_estimates_df]).fillna(0.)
        ...
        for item_name, item_data in responses_df.iloc[g.indx,
            ...
            if i_n:
                ...
                for r in i.resp_cats:
                    responses[(i.name, r, g.name)] = item_data.loc[item_data == r].index.tolist()
        g.items = g_items
...
def likelihood_long(items, groups, responses, theta):
    ...
    for g in groups.values():
        for item in g.items.keys():
            i = items[item]
            for r in i.resp_cats:
                indx = responses[(i.name, r, g.name)]
                if len(indx):
...
def dram_mcmc(items, groups, responses, theta_t, proposal_sd, prior_mean, prior_sd):
...
def multilevel(data, theta_var, school_id, student_id, exam_var, fixed_effects=[]):
    ...
    model = sm.MixedLM.from_formula(theta_var + ' ~ ' + fixed,
                                    ...
                                    vc_formula = vc,
                                    data = data)
    print('Estimating MixedLM (', datetime.now().strftime("%Y.%m.%d %H:%M:%S"), "):")
    print('sm.MixedLM.from_formula({}  ~ {},'.format(theta_var,fixed))
    print('                        groups={},'.format(school_id))
    print('                        re_formula=1 + {},'.format(exam_var))
    print('                        vc_formula={},'.format(vc))
    print('                        data=data)')
    results = model.fit(maxiter = 50)
    print('MixedLM estimation has finished (', datetime.now().strftime("%Y.%m.%d %H:%M:%S"), ').')
    return model, results

def rescale_priors( prior_mean, prior_sd , groups):

    theta_test = np.random.normal(prior_mean, prior_sd) # TDL - add condition to draw at least N

    for g in groups.values():
        _mean = theta_test[g.indx].mean()
        _sd = theta_test[g.indx].std()
...
def generate_pv_child(n_draw, max_indep_chains, shift, csvname=''):
...
def generate_pv_multipro(items, groups, responses, estimates, njobs=2,
    ...
    pool_res =   [pool.apply_async(generate_pv_child, args=(n_draw, max_indep_chains, shift, csvname))
...
def generate_pv_single(items, groups, responses, estimates,
    ...
    for chain in range(max_chain):
        ...
        theta_t, proposal_sd = eap(_items, _groups, responses, 195)  # paranoid nip - TDL scale it down at some time
        ...
        for _ in range(burn0):
            theta_t = dram_mcmc(_items, _groups, responses, theta_t, proposal_sd, prior_mean, prior_sd)
            # if shift==0:
            #     progress_bar.update()
        ...
        if len(mlv_args):
            ...
            for _ in range(burn1):
                ...
                mlv_data.to_csv('mlv_data{}.csv'.format(shift), index=False) # dump danych
        ...
        for draw in range(max(chain_draws[chain]) + 1):
            ...
            if draw in chain_draws[chain]:
                ...
                if len(mlv_args):
                    ...
                    prior_mean, prior_sd = rescale_priors(prior_mean, prior_sd,
                                                          _groups)
                    with open(csvname+"regrsummary"+str(shift)+".txt", "w") as f:
                      print(mlv_results.summary(), file=f)
